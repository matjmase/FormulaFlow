...
{
    public class NumberFeedbackIntermediateCard : IntermediateFormulaCard
    {
        public override NetworkCardType Type => NetworkCardType.FeedbackNumber;
        ...
        public override string Label => "Number Feedback Formula";

        public override string Description => "Create formulas to transform the numeric input to the output";

        public override string DefaultName => "Number Feedback Formula";

        protected override IntermediateParameter[] _defaultParameters => [
                new NumberFeedbackFormulaIntermediateParameter(0),
                new BufferInputIntermediateParameter(1),
                new NumericInputIntermediateParameter(2),
            ];

        public override async Task<Dictionary<int, int>> GetBuffer()
        {
            var param = (Parameters[0] as NumberFeedbackFormulaIntermediateParameter);
        ...
        public override async Task<BackTestResultDto[]> Process(BackTestResultDto[][] inputs, INoSqlRepository<StockDataEntry> dataRepo, DateTime start, DateTime end, int totalBuffer)
        {
            ...
            var param = (Parameters[2] as NumericInputIntermediateParameter);
            ...
            var seed = double.Parse(param.Value);
            ...
            for (var i = 0; i < output.Length; i++)
            {
                ...
                var date = GetDate(inputs, buffers, i + univBuffer);
